...
import { Users, Sparkles, DollarSign, Brain, BarChart3, GraduationCap, type LucideIcon } from "lucide-react";
import { Skeleton } from "@/components/ui/skeleton";
import { LandingNavbar } from "@/components/layout/LandingNavbar";
import type { SitePage, BecomeGuruContent, TitleDescriptionItem } from "@shared/schema";
...
const iconMap: Record<string, LucideIcon> = {
  "Reach Thousands": Users,
  "Earn Revenue": DollarSign,
  "AI-Powered Tools": Brain,
  "Full Analytics": BarChart3,
};

const ACCENT_COLORS = ["#00F5FF", "#7C3AED", "#F59E0B", "#10B981"];
...
export default function BecomeGuru() {
  const { data: page, isLoading } = useQuery<SitePage>({
    queryKey: ["/api/pages", "become-guru"],
    queryFn: () => fetch("/api/pages/become-guru").then((r) => r.json()),
  });

  const content = (page?.content || {}) as BecomeGuruContent;
  ...
  const benefits: TitleDescriptionItem[] = content.benefits || [];
        ...
        <section className="relative overflow-hidden pt-14 pb-10 md:pt-20 md:pb-14">
          ...
          <div className="max-w-4xl mx-auto px-4 md:px-8 relative z-10 text-center">
            <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              >
                {content.heroHeading || "Become a Guru on OurShiksha"}
              </h1>

              <p className="text-base md:text-lg max-w-2xl mx-auto" style={{ color: C.textSecondary }}
                data-testid="text-hero-subtext">
                {content.heroSubtext || "Share your expertise and impact thousands of learners."}
              </p>
            </motion.div>
          </div>
        </section>
        ...
        {benefits.length > 0 && (
          <section className="py-8 md:py-12 relative overflow-hidden">
            ...
            <div className="max-w-5xl mx-auto px-4 md:px-8 relative z-10">
              ...
              >
                Why Teach on OurShiksha?
              </motion.h2>

              <div className="grid md:grid-cols-2 gap-5">
                {benefits.map((benefit: TitleDescriptionItem, i: number) => {
                  const Icon = iconMap[benefit.title] || Sparkles;
                  ...
                  return (
                    ...
                    >
                      ...
                      <h3 className="font-semibold mb-2 text-sm md:text-base" style={{ color: C.textPrimary }}>{benefit.title}</h3>
                      <p className="text-sm leading-relaxed" style={{ color: C.textSecondary }}>{benefit.description}</p>
                    </motion.div>
                  );
                })}
              </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <Link href="/signup?role=guru">
                ...
                >
                  ...
                  {content.ctaText || "Apply to Become a Guru"}
                </button>
              </Link>
            </motion.div>
          </div>

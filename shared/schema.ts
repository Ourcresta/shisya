...
export const mithraConversations = pgTable("mithra_conversations", {
  ...
  userId: varchar("user_id", { length: 36 }).notNull().references(() => users.id),
  courseId: integer("course_id").notNull(),
  pageType: varchar("page_type", { length: 20 }).notNull(), // lesson, lab, project, test_prep
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const mithraMessages = pgTable("mithra_messages", {
  ...
  responseType: varchar("response_type", { length: 20 }), // explanation, hint, guidance, warning
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
// Mithra response schema
export interface MithraResponse {
  answer: string;
  type: MithraResponseType;
}

// Mithra page types where it's allowed
